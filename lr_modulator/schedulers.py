...
class BatchBaseSchedule:
    ...
    def lr_at(self, step_idx: int) -> float:
        step_idx = min(step_idx, self.total_steps)
...
class EMALossModulator:
    ...
    def __init__(
        ...
        self.kernel = [(config.rho ** (m - 1)) / z for m in range(1, config.m_win + 1)]

        self.raw_abs_ema = None
        ...
        self.last_raw = 0.0
        self.last_delta = 0.0
        self.last_base_lr = self.base.current_lr
        self.last_mod_lr = self.base.current_lr
    ...
    def _beta_eff(self) -> float:
        if not self.config.use_auto_beta:
            return self.config.beta_fixed

        if self.raw_abs_ema is None or self.raw_abs_ema < 1e-8:
            return min(self.config.beta_cap, max(self.config.beta_fixed, 0.5))
    ...
    def on_batch_end(self, loss_value: float) -> None:
        self.ema = float(loss_value) if self.ema is None else self.config.alpha * self.ema + (1.0 - self.config.alpha) * float(loss_value)
        u = self.phi(self.ema)
        self.hist.append(u)
        ...
        if self.batch_idx >= self.config.warmup_steps and len(self.hist) >= self.config.m_win + 1:
            raw_t = self._raw(u)
            abs_raw = abs(raw_t)
        ...
        self.last_raw = raw_t
        self.last_delta = delta_t
        self.last_beta_eff = beta_eff
        self.last_mod_lr = self.last_base_lr * (1.0 + delta_t)
    ...
    def stats(self) -> Dict[str, float]:
        ...
        return {
            "clip_rate": clip_rate,
            "delta_mean_abs_final": delta_mean_abs,
...
class Controller:
    def __init__(
        ...
        self.method = method
        if method in {"constant", "step", "cosine", "onecycle"}:
            self.kind = "base"
            self.base = BatchBaseSchedule(
                optimizer=optimizer,
                config=config,
            ...
            self.last_delta = 0.0
            self.last_raw = 0.0
        elif method in {"ours_cosine", "ours_onecycle"}:
            self.kind = "mod"
            base_mode = "cosine" if method == "ours_cosine" else "onecycle"
